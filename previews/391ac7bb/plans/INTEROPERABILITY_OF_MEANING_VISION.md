...
# Evidence note --- BPMNSM, 18 September 2026

BPMNSM has added concrete evidence to the vision's Preserve → Characterize → Map → Project trajectory.

The demonstrator now separates a Business Object identity from its BPMN representation identity, supports typed Business Objects, persists Business Object ↔ BPMN representation links, and restores those links after reopening the repository.

This does not prove a universal business ontology or a distributed semantic repository. It does provide a practical test bed for the next Interoperability of Meaning question: how meaning should be governed when a business object or relation spans several representations, or has no BPMN representation at all.

The appropriate next experiment is therefore not forced unification. It is to determine which information belongs natively in BPMN and which information, if any, requires a complementary repository-level representation while preserving the source models.
