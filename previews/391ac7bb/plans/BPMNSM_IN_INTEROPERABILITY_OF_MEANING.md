...
# 23. Evidence update --- 18 September 2026

The demonstrated foundations now extend beyond repository semantics and Environment projection.

BPMNSM has demonstrated a minimal Business Object identity distinct from BPMN element identity, with multiple type references, explicit attachment/detachment to BPMN representations, repository persistence and restoration after reopening.

This is important for Interoperability of Meaning because it separates:
...
from
BPMN occurrence/context
```

without replacing BPMN as the process exchange language.

The result also identifies a new research boundary. The current canonical repository can persist Business Objects and representation links in BPMN XML, but this does not yet solve business objects with no BPMN representation, cross-model Business Relations or contextualization across multiple BPMN documents.

Those cases should be used to test whether a complementary repository-level artifact is required. Such an artifact is not yet implemented and must not become a proprietary replacement for the BPMN documents themselves.

The product distribution chain is now demonstrated through standalone Viewer/Editor, an offline demonstration package and GitHub Pages. The public Viewer has also been verified manually for BPMN file opening.
