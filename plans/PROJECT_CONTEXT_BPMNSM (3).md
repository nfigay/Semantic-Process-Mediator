...
Le chantier datatype / widget / persistance est terminé. Le dernier baseline complet explicitement confirmé est :
...
Depuis le chantier datatype, les éléments suivants sont également considérés comme démontrés :
...
Décision fonctionnelle actuelle pour l'explorateur :

> **La position d'un processus dans l'arbre n'est pas une propriété intrinsèque du processus. Elle est le résultat d'une projection construite à partir de ses classifications, de ses relations et du profil de vue actif.**

Le premier profil concret reste **CoC-centric**, car le besoin prioritaire est la rationalisation de référentiels de processus par des Centres de Compétence du domaine spatial.

La généralisation future par `ClassificationScheme`, `Category`, `ClassificationAssignment` et `ProjectionProfile` est **prévue mais ne doit pas être implémentée prématurément**.

Prochaine étape technique naturelle lorsque le travail UI reprend : vérifier sur les fichiers sources courants la transition :
...
et démontrer que le chargement d'un modèle :

``` text
cache Welcome
montre le canvas BPMN
montre Properties
montre la palette en mode editor
active les commandes BPMN de la toolbar
```

Ne pas reconstruire `create-app.js` ou `layout.js` depuis ce document : demander/lire leur version exacte avant modification.
